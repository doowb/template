...
describe('Views', function () {
  ...
  it('should pick an option from the `app.options`', function () {
    ...
    var views = new Views();
    views.app = app;
    var pickOption = views.pickOption;
    views.pickOption = function (key) {
  ...
  it('should get the current items on the views', function () {
    var views = createViews();
    views.set('foo', createView({path: 'foo', content: 'foo'}));
    views.set('bar', createView({path: 'bar', content: 'bar'}));
    views.set('baz', createView({path: 'baz', content: 'baz'}));
    views.set('bang', createView({path: 'bang', content: 'bang'}));
  ...
  it('should set the items on the views', function () {
    var views = createViews();
    var items = {};
    items.foo = createView({path: 'foo', content: 'foo'});
    items.bar = createView({path: 'bar', content: 'bar'});
    items.baz = createView({path: 'baz', content: 'baz'});
    items.bang = createView({path: 'bang', content: 'bang'});
    views.items = items;
    assert.deepEqual(Object.keys(items), ['foo', 'bar', 'baz', 'bang']);
    assert.deepEqual(Object.keys(views.items), ['foo', 'bar', 'baz', 'bang']);
    forOwn(items, function (item, key) {
      assert.deepEqual(item, views.get(key));
    });
  });
  ...
  it('should sort the items by the key', function () {
    ...
    Object.keys(views.items).should.eql(['foo', 'bar', 'baz', 'bang']);
    views.sortBy();
    Object.keys(views.items).should.eql(['bang', 'bar', 'baz', 'foo']);
  });

  it('should sort the items by a property', function () {
    var views = createViews();
    views.set('foo', createView({name: 'a-foo', path: 'a-foo.hbs', content: 'foo'}));
    views.set('bar', createView({name: 'y-bar', path: 'y-bar.hbs', content: 'bar'}));
    views.set('baz', createView({name: 'x-baz', path: 'x-baz.hbs', content: 'baz'}));
    views.set('bang', createView({name: 'w-bang', path: 'w-bang.hbs', content: 'bang'}));
    Object.keys(views.items).should.eql(['foo', 'bar', 'baz', 'bang']);

    views.sortBy('name');
    Object.keys(views.items).should.eql(['foo', 'bang', 'baz', 'bar']);
  });

  it('should be chainable', function () {
    var views = createViews();
    views.set('foo', createView({path: 'a-foo', order: '20', content: 'foo'}));
    views.set('bar', createView({path: 'y-bar', order: '10', content: 'bar'}));
    views.set('baz', createView({path: 'x-baz', order: '30', content: 'baz'}));
    views.set('bang', createView({path: 'w-bang', order: '40', content: 'bang'}));
    assert.deepEqual(Object.keys(views.items), ['foo', 'bar', 'baz', 'bang']);
    views
      .sortBy('name')
      .sortBy('order');

    views.items.should.have.properties(['bar', 'foo', 'baz', 'bang']);
  });

  it('should get recent items returned as a List', function () {
    var views = createViews();
    views.set('post-1', createView({date: '2015-01-05', path: 'Post 1', content: 'Post 1'}));
    views.set('post-2', createView({date: '2015-02-05', path: 'Post 2', content: 'Post 2'}));
    views.set('post-3', createView({date: '2015-03-05', path: 'Post 3', content: 'Post 3'}));
    views.set('post-4', createView({date: '2015-04-05', path: 'Post 4', content: 'Post 4'}));
    views.set('post-5', createView({date: '2015-05-05', path: 'Post 5', content: 'Post 5'}));
    views.set('post-6', createView({date: '2015-06-05', path: 'Post 6', content: 'Post 6'}));
    views.set('post-7', createView({date: '2015-07-05', path: 'Post 7', content: 'Post 7'}));
